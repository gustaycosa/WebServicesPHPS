<?php
    ...
    $query = "dbo.RPT_SP_FACTURASDIARIA_DET @fini = N'".$_GET['x6']."', @cliente = N'".$_GET['x3']."', @ffin = N'".$_GET['x2']."', @suc = N'".$_GET['x1']."', @tipo = N'".$_GET['x5']."', @user = N'".$_GET['x4']."', @emp = N'".$_GET['x7']."'";
    //print_r($query);
    ...
    if (0 !== sqlsrv_num_rows($cadena)){
         while($row = sqlsrv_fetch_array($cadena) ){ 
            ...
            $data['importe_costo'] = utf8_encode($row['importe_costo']);
            $data['poliza'] = utf8_encode($row['estatus']);
            $data['id_cuentactb'] = utf8_encode($row['id_cuentactb']);
            $data['debepoliza'] = utf8_encode($row['debepoliza']);
            $data['haberpoliza'] = utf8_encode($row['haberpoliza']);
